...
import (
	...
	"fmt"
	"io"
	"jsonrpc-golden-pp-cli/internal/cliutil"
	"jsonrpc-golden-pp-cli/internal/config"
	"math"
	"net/http"
	"net/url"
	"os"
...
func (c *Client) nextJSONRPCID() int64 {
	c.jsonRPCMu.Lock()
	defer c.jsonRPCMu.Unlock()
	c.jsonRPCID++
	return c.jsonRPCID
}

func (c *Client) previewJSONRPCID() int64 {
	c.jsonRPCMu.Lock()
	defer c.jsonRPCMu.Unlock()
	return c.jsonRPCID + 1
}
...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "jsonrpc-golden-pp-cli", "http")
...
func (c *Client) doInternal(ctx context.Context, method, path string, params map[string]string, body any, headerOverrides map[string]string, readOnlyIntent bool) (json.RawMessage, int, error) {
	...
	// JSON-RPC: all requests are POSTed to BaseURL with a JSON-RPC envelope.
	targetURL := c.BaseURL
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if bodyBytes != nil {
			if err := json.Unmarshal(bodyBytes, &arguments); err != nil {
				return nil, 0, fmt.Errorf("parsing JSON-RPC params: %w", err)
			}
		}
		...
		if err != nil {
			return nil, 0, fmt.Errorf("marshaling JSON-RPC envelope: %w", err)
		}
		req, err := http.NewRequestWithContext(ctx, http.MethodPost, targetURL, bytes.NewReader(envelopeBytes))
		if err != nil {
		...
		if req.Header.Get("User-Agent") == "" {
			req.Header.Set("User-Agent", "jsonrpc-golden-pp-cli/0.1.0")
		}
		...
		if resp.StatusCode < 400 {
			...
			if err := json.Unmarshal(sanitizeJSONResponse(respBody), &jsonRPC); err != nil {
				return nil, resp.StatusCode, fmt.Errorf("parsing JSON-RPC response: %w", err)
			}
			if jsonRPC.Error != nil {
				return nil, resp.StatusCode, fmt.Errorf("JSON-RPC error %s: %s", string(jsonRPC.Error.Code), jsonRPC.Error.Message)
			}
			if jsonRPC.Result == nil {
				return nil, resp.StatusCode, fmt.Errorf("parsing JSON-RPC response: missing result")
			}
			c.limiter.OnSuccess()
			...
			return jsonRPC.Result, resp.StatusCode, nil
		}
...
func (c *Client) dryRun(method, targetURL, path string, params map[string]string, body []byte, headerOverrides map[string]string, authHeader string) (json.RawMessage, int, error) {
	arguments := map[string]any{}
	if body != nil {
		_ = json.Unmarshal(body, &arguments)
	}
	for key, value := range params {
		if value != "" {
			arguments[key] = value
...
func authPlaceholderCredentialError(cfg *config.Config) error {
	return authPlaceholderCredentialErrorWithSetup(cfg, "export JSONRPC_GOLDEN_TOKEN=<your-token> or jsonrpc-golden-pp-cli auth set-token <token>")
}
...
func (c *Client) maskCredentialText(text string, extraCredentials ...string) string {
	...
	if c != nil && c.Config != nil {
		...
		addCredential(c.Config.JsonrpcGoldenToken)
	}
