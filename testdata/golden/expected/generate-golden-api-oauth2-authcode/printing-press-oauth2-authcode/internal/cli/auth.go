...
func newAuthLoginCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return runOAuthLogin(cmd, flags, clientID, clientSecret, port, clientIDFromFlag, clientSecretFromFlag, redirectOverride)
		},
	}
	...
	cmd.Flags().StringVar(&redirectOverride, "redirect-uri", os.Getenv("PRINTING_PRESS_OAUTH2_REDIRECT_URI"), "Registered OAuth2 redirect URI for providers that reject loopback callbacks; must forward the callback query string to http://127.0.0.1:<port>/callback")

	return cmd
}

// validateRedirectOverride rejects unsafe --redirect-uri values before any
// auth code can be routed through them: the URI must be absolute, carry no
// fragment or userinfo, and use https unless the host is loopback. A typo or
// poisoned redirect-URI environment variable could otherwise send the
// authorization code through an unintended endpoint or downgrade it to
// cleartext.
func validateRedirectOverride(raw string) error {
	u, err := url.Parse(raw)
	if err != nil {
		return fmt.Errorf("invalid --redirect-uri %q: %w", raw, err)
	}
	if !u.IsAbs() || u.Host == "" {
		return fmt.Errorf("invalid --redirect-uri %q: must be an absolute URL with a host", raw)
	}
	if u.Fragment != "" || u.User != nil {
		return fmt.Errorf("invalid --redirect-uri %q: fragments and userinfo are not allowed", raw)
	}
	host := u.Hostname()
	loopback := host == "localhost" || host == "127.0.0.1" || host == "::1"
	if u.Scheme != "https" && !(u.Scheme == "http" && loopback) {
		return fmt.Errorf("invalid --redirect-uri %q: must use https (http is only allowed for localhost)", raw)
	}
	return nil
}

func runOAuthLogin(cmd *cobra.Command, flags *rootFlags, clientID, clientSecret string, port int, clientIDFromFlag, clientSecretFromFlag bool, redirectOverride string) error {
	w := cmd.OutOrStdout()
	if redirectOverride != "" {
		if err := validateRedirectOverride(redirectOverride); err != nil {
			return err
		}
	}
	...
	if cliutil.IsVerifyEnv() {
		if redirectOverride != "" {
			params.Set("redirect_uri", redirectOverride)
		} else {
	...
	// "localhost" redirect URIs outright. When --redirect-uri is set, that
	// registered URI is sent to the provider instead; the local listener still
	// receives the forwarded callback.
	...
	// Some token endpoints require an explicit JSON response preference and
	// fail non-obviously when it is absent; other providers ignore it.
