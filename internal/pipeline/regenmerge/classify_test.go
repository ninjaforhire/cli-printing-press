...
func TestClassifyGeneratedInputRefreshTakesFresh(t *testing.T) {
	t.Parallel()
	...
	require.NoError(t, os.WriteFile(filepath.Join(pubDir, rel), []byte(header+"var whichIndex = []string{\"old cmd\"}\n"), 0o644))
	require.NoError(t, os.WriteFile(filepath.Join(freshDir, rel), []byte(header+"var whichIndex = []string{\"new cmd\"}\n"), 0o644))

	report, err := Classify(pubDir, freshDir, Options{Force: true})
	require.NoError(t, err)
	assert.Equal(t, VerdictTemplatedClean, verdictMap(report)[filepath.ToSlash(rel)],
		"research-driven which.go must take fresh even when the older copy still compiles")
}

func TestClassifyBaseDirTreatsUnpatchedRewriteAsClean(t *testing.T) {
	t.Parallel()
	...
	header := "// Generated by CLI Printing Press. DO NOT EDIT.\npackage client\n\n"
	oldBody := header + `func credentialAppliesToURL(u string) bool { return u != "" }
`
	pubBody := header + `// local note
func credentialAppliesToURL(u string) bool { return u != "" }
`
	freshBody := header + `func credentialAppliesToURL(u string) bool { return u == "https://bound.example" }
`
	for _, root := range []string{baseDir, pubDir, freshDir} {
		require.NoError(t, os.MkdirAll(filepath.Join(root, filepath.Dir(rel)), 0o755))
	}
	require.NoError(t, os.WriteFile(filepath.Join(baseDir, rel), []byte(oldBody), 0o644))
	require.NoError(t, os.WriteFile(filepath.Join(pubDir, rel), []byte(pubBody), 0o644))
	require.NoError(t, os.WriteFile(filepath.Join(freshDir, rel), []byte(freshBody), 0o644))

	report, err := Classify(pubDir, freshDir, Options{Force: true, BaseDir: baseDir})
	require.NoError(t, err)
	assert.Equal(t, VerdictTemplatedClean, verdictMap(report)[filepath.ToSlash(rel)],
		"a rewritten body that still matches the original emission is version drift; fresh wins")
}

func TestClassifyBaseDirKeepsHandEditedDeclAsDrift(t *testing.T) {
	t.Parallel()
...
`
	pubBody := header + `func credentialAppliesToURL(u string) bool { return u != "" }
...
`
	...
	require.Equal(t, VerdictTemplatedValueDrift, got,
		"a decl that no longer matches the original emission is still a hand-edit")
	var fc *FileClassification
	for i := range report.Files {
		if report.Files[i].Path == filepath.ToSlash(rel) {
			fc = &report.Files[i]
			break
		}
	}
	require.NotNil(t, fc.ValueDrift)
	_, rewritten := fc.ValueDrift.Decls["credentialAppliesToURL"]
	assert.False(t, rewritten, "unpatched rewrite must not stay in value-drift")
	_, patched := fc.ValueDrift.Decls["HandPatched"]
	assert.True(t, patched, "hand-edited decl must remain value-drift")
}
