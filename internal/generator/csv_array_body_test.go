...
func TestGenerateCSVArrayBodyFields(t *testing.T) {
	...
	require.Contains(t, code, `bodyMap["emails"] = parsedEmails`)
	require.Contains(t, code, `parsedTags, parseErr := cliutil.ParseStringList(bodyTags)`)
	require.Contains(t, code, `bodyMap["tags"] = parsedTags`)
	require.Contains(t, code, `bodyMap["attendees"] = cliutil.CSVTemplateObjects(bodyAttendees, map[string]any{"emailAddress": map[string]any{"address": "$value"}, "type": "required"})`)
	require.Contains(t, code, `bodyMap["subject"] = bodySubject`)
